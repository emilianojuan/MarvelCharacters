    ...
    func fetchCharacters(pageNumber: Int, pageSize: Int, nameStartsWith: String?) -> AnyPublisher<CharactersPage, Error> {
        ...
        .map { response in
            guard let data = response.data,
                ...
                return CharactersPage(page: 0, searchText: nameStartsWith, totalPages: 0, totalItems: 0, characters: [])
            }
            let pages = Int(ceil(Double(total) / Double(pageSize)))
            return CharactersPage(page: pageNumber,
                                  searchText: nameStartsWith,
                                  totalPages: pages,
                                  totalItems: total,
                                  characters: results.map { $0.character })
